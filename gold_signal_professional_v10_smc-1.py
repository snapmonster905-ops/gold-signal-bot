"""
Gold Trading Signals — Professional V10 Telegram Bot
====================================================
GitHub Actions / Telegram edition.

V10 Professional 14-Layer architecture:
...
IMPORTANT:
  No strategy can guarantee a particular win rate. V10 is designed to improve
...
# Smart Money Concepts (SMC) sequence gate.
# Required order: liquidity sweep -> MSS -> FVG -> FVG retest/reaction.
SMC_LOOKBACK = 2
SMC_SEARCH_BARS = 36
SMC_MAX_BARS_AFTER_SWEEP = 12
SMC_MAX_BARS_AFTER_MSS = 8
SMC_FVG_MIN_ATR = 0.05
SMC_ENTRY_AT_FVG_MID = True
SMC_REQUIRE_FVG_RETEST = True
SMC_SWEEP_BUFFER_ATR = 0.02
...
def send_welcome_message():
    text = (
        ...
        "🧠 <b>Smart decision:</b> weighted confirmation + mandatory SMC sequence + safety filters.\n"
...
def liquidity_sweep(candles, pdh, pdl):
    """Legacy PDH/PDL sweep layer: rejection of previous-day high/low."""
...
def _latest_confirmed_swing_before(candles, end_index, direction, lookback=2):
    """Return the most recent confirmed swing high/low before end_index."""
    if end_index < lookback * 2 + 1:
        return None
    start = max(lookback, end_index - 30)
    for i in range(end_index - 1, start - 1, -1):
        left = candles[i-lookback:i]
        right = candles[i+1:min(len(candles), i+lookback+1)]
        if len(left) < lookback or len(right) < lookback:
            continue
        if direction == "LOW":
            value = candles[i]["l"]
            if value <= min(x["l"] for x in left + right):
                return i, value
        else:
            value = candles[i]["h"]
            if value >= max(x["h"] for x in left + right):
                return i, value
    return None


def _fvg_candidates(candles, start_index, end_index, atr_value):
    """Return FVGs formed after start_index, oldest to newest."""
    gaps = []
    if not atr_value:
        return gaps
    gap_min = max(atr_value * SMC_FVG_MIN_ATR, 0.01)
    lo = max(2, start_index)
...
def smc_sequence(candles, atr_value, lookback=SMC_LOOKBACK):
    """
    Exact SMC sequence used by the supplied reference images:
      1) liquidity sweep of a confirmed swing,
      2) MSS/structure break in the opposite direction,
      3) FVG created after MSS,
      4) price retests/reacts from that FVG.

    This is deliberately separate from the legacy PDH/PDL layer.  The
    sequence becomes a mandatory gate for a BUY/SELL signal when enabled.
    """
    result = {
        "direction": "NEUTRAL",
        "sweep_index": None,
        ...
        "entry_zone": None,
        "retest": False,
        "reason": "No complete sweep -> MSS -> FVG sequence",
    }
    if len(candles) < 15 or not atr_value:
        return result

    n = len(candles)
    start = max(lookback * 2 + 1, n - SMC_SEARCH_BARS)
    sweep_buffer = atr_value * SMC_SWEEP_BUFFER_ATR

    # Search newest-to-oldest so the active/latest setup wins.
    for s in range(n - 2, start - 1, -1):
        # ----- Bullish setup: sweep a prior swing low, then break a prior swing high.
        swing_low = _latest_confirmed_swing_before(candles, s, "LOW", lookback)
        if swing_low:
            _, low_level = swing_low
            sc = candles[s]
            if sc["l"] < low_level - sweep_buffer and sc["c"] > low_level:
                max_mss = min(n - 1, s + SMC_MAX_BARS_AFTER_SWEEP)
                # MSS must break a swing high that existed before the sweep.
                pre_high = _latest_confirmed_swing_before(candles, s, "HIGH", lookback)
                if pre_high:
                    _, high_level = pre_high
                    for m in range(s + 1, max_mss + 1):
                        if candles[m]["c"] > high_level:
                            max_fvg = min(n - 1, m + SMC_MAX_BARS_AFTER_MSS)
                            gaps = [
                                g for g in _fvg_candidates(candles, m + 1, max_fvg, atr_value)
                                if g["direction"] == "BULLISH"
                            ]
                            if gaps:
                                last = candles[-1]
                                # Prefer the newest FVG that is actually being
                                # retested; this avoids ignoring a valid earlier FVG
                                # just because a later gap formed and was not touched.
                                selected = None
                                for candidate in reversed(gaps):
                                    f_low, f_high = candidate["low"], candidate["high"]
                                    retest = (
                                        f_low <= last["c"] <= f_high
                                        or f_low <= last["h"] and last["l"] <= f_high
                                    )
                                    if not SMC_REQUIRE_FVG_RETEST or retest:
                                        selected = (candidate, retest)
                                        break
                                if selected:
                                    gap, retest = selected
                                    ...
                                    result.update({
                                        ...
                                        "mss_index": m,
                                        "mss_level": high_level,
                                        "fvg_low": f_low,
                                        "fvg_high": f_high,
                                        "fvg_index": gap["formed_at"],
                                        "entry_zone": (f_low, f_high),
                                        "retest": retest,
                                        "reason": "Bullish liquidity sweep -> MSS -> FVG retest",
                                    })
                                    return result
                            break

        # ----- Bearish setup: sweep a prior swing high, then break a prior swing low.
        swing_high = _latest_confirmed_swing_before(candles, s, "HIGH", lookback)
        if swing_high:
            _, high_level = swing_high
            sc = candles[s]
            if sc["h"] > high_level + sweep_buffer and sc["c"] < high_level:
                max_mss = min(n - 1, s + SMC_MAX_BARS_AFTER_SWEEP)
                pre_low = _latest_confirmed_swing_before(candles, s, "LOW", lookback)
                if pre_low:
                    _, low_level = pre_low
                    for m in range(s + 1, max_mss + 1):
                        if candles[m]["c"] < low_level:
                            max_fvg = min(n - 1, m + SMC_MAX_BARS_AFTER_MSS)
                            gaps = [
                                g for g in _fvg_candidates(candles, m + 1, max_fvg, atr_value)
                            ...
                            if gaps:
                                ...
                                if selected:
                                    ...
                                    result.update({
                                        ...
                                        "entry_zone": (f_low, f_high),
                                        "retest": retest,
                                        "reason": "Bearish liquidity sweep -> MSS -> FVG retest",
                                    })
                                    return result
                            break
...
def market_structure_signal(candles, lookback=2):
    """Legacy BOS/structure trend layer."""
...
def fair_value_gap_signal(candles, atr_value):
    """Three-candle FVG context, plus a recent reaction."""
...
def analyze(candles, htf_candles):
    ...
    smc = smc_sequence(candles, atr_value)
    ...
    smc_bull = smc["direction"] == "BULLISH"
    smc_bear = smc["direction"] == "BEARISH"
    smc_gate_bull = (not SMC_REQUIRE_FVG_RETEST) or smc_bull
    smc_gate_bear = (not SMC_REQUIRE_FVG_RETEST) or smc_bear

    if safety_ok and smc_gate_bull and bullish_score >= MIN_SETUP_SCORE and weighted_bull >= MIN_WEIGHTED_SCORE * total_weight:
        if weighted_bull > weighted_bear and edge >= 0.18:
            signal = "BUY"
    elif safety_ok and smc_gate_bear and bearish_score >= MIN_SETUP_SCORE and weighted_bear >= MIN_WEIGHTED_SCORE * total_weight:
    ...
    if signal == "HOLD":
        ...
        if SMC_REQUIRE_FVG_RETEST and smc["direction"] == "NEUTRAL":
            reasons.append("SMC sequence not complete: sweep -> MSS -> FVG retest")
        if not reasons:
    ...
    return {
        ...
        "smc_sweep_level": smc["sweep_level"],
        "smc_mss_level": smc["mss_level"],
        "smc_fvg_low": smc["fvg_low"],
        "smc_fvg_high": smc["fvg_high"],
        "smc_fvg_index": smc["fvg_index"],
        "smc_retest": smc["retest"],
        "order_block": ob,
...
def trade_levels(a):
    ...
    # For a confirmed SMC setup, use the FVG midpoint as the planned entry.
    # If the FVG is unavailable, fall back to the current market price.
    entry_price = price
    if signal in ("BUY", "SELL") and SMC_ENTRY_AT_FVG_MID:
        fvg_low = a.get("smc_fvg_low")
        fvg_high = a.get("smc_fvg_high")
    ...
    if direction == "SELL":
        ...
        atr_stop = entry_price + SL_ATR_MULT * atr_value
        if structural_stop and structural_stop > entry_price:
            sl = max(atr_stop, structural_stop)
        else:
            sl = atr_stop
        max_sl = entry_price + MAX_RISK_ATR * atr_value
        sl = min(sl, max_sl)
        risk = max(sl - entry_price, 0.01)
        t1 = entry_price - max(T1_ATR_MULT * atr_value, risk * T1_RR_FLOOR)
        t2 = entry_price - max(T2_ATR_MULT * atr_value, risk * MIN_RR_T2)
    else:
        ...
        if structural_stop and structural_stop < entry_price:
            ...
            sl = atr_stop
        min_sl = entry_price - MAX_RISK_ATR * atr_value
        sl = max(sl, min_sl)
        risk = max(entry_price - sl, 0.01)
        t1 = entry_price + max(T1_ATR_MULT * atr_value, risk * T1_RR_FLOOR)
        t2 = entry_price + max(T2_ATR_MULT * atr_value, risk * MIN_RR_T2)

    rr1 = abs(t1 - entry_price) / max(abs(entry_price - sl), 0.01)
    rr2 = abs(t2 - entry_price) / max(abs(entry_price - sl), 0.01)
    return {
        "entry": entry_price,
        "sl": sl,
        "t1": t1,
...
def format_alert(a):
    ...
    return (
        f"{emoji} <b>GOLD V10 • {signal} CONFIRMED</b>\n"
        ...
        f"🧠 <b>SMC:</b> {esc(a['smc_direction'])} • Retest {'YES' if a['smc_retest'] else 'NO'}\n"
...
def format_status_reply(a):
    ...
    return (
        ...
        f"🧠 SMC Sequence: {esc(a['smc_direction'])} • Retest {'YES' if a['smc_retest'] else 'NO'}\n"
...
def format_signal_reply(a):
    if a["signal"] == "BUY":
        ...
        reason = "Bullish weighted confirmation passed the V10 threshold."
    elif a["signal"] == "SELL":
        headline = "🔴 <b>SELL CONFIRMED</b>"
        score = a["bearish_score"]
        reason = "Bearish weighted confirmation passed the V10 threshold."
    else:
    ...
    return (
        ...
        f"🧠 SMC: {esc(a['smc_direction'])} • Retest {'YES' if a['smc_retest'] else 'NO'}\n"
        f"🛡️ Safety: {'PASS' if a['safety_ok'] else 'BLOCK'}\n\n"
        f"{esc(reason)}\n\n"
        f"🕒 {stamp()}"
    )
...
def format_layers_reply(a):
    lines = ["🧠 <b>V10 — 14-LAYER + SMC GOLD ENGINE</b>\n"]
...
def format_help():
    lines = ["📚 <b>GOLD SIGNALS V10 — COMMAND CENTER</b>\n"]
    for command, description in COMMANDS.items():
        lines.append(f"{command} — {description}")

    lines.extend([
        "",
        "🧠 <b>V10 engine:</b> 14-layer weighted confirmation + SMC sequence gate.",
        "💧 SMC entry sequence: Liquidity Sweep → MSS → FVG → FVG Retest.",
        "📍 Confirmed SMC entries use the FVG midpoint as the planned entry.",
...
def format_why_reply(a):
    ...
    return (
        ...
        f"📦 FVG: {esc(a['fvg'])}\n"
        f"🧠 SMC sequence: {esc(a['smc_direction'])} — {esc(a['smc_reason'])}\n"
...
def main():
    ...
    print("=" * 60)
    print("GOLD SIGNAL BOT V10 — RUN START")
    ...
    if result is None:
        ...
        if commands:
            send_telegram_message(
                ...
                "to calculate all V10 layers yet."
            )
    ...
    print("[DONE] V10 state saved successfully.")
