...
def send_welcome_message():
    """Send a simple Telegram welcome message when /start is received."""
    text = (
        "👋 *Welcome to Gold Trading Signals!*\n\n"
        "🤖 Your XAU/USD signal bot is connected.\n\n"
        "Commands:\n"
        "• /start — welcome + current status\n"
        "• status — current market status\n"
        "• target — current entry / SL / T1\n"
        "• signal — current signal\n"
        "• price — current gold price\n\n"
        "⏱️ The bot checks the market every 15 minutes.\n"
        "📊 Signals use EMA-50, MACD, RSI, VWAP and ADX.\n\n"
        "⚠️ Technical view only — not financial advice."
    )
...
def get_new_updates(last_update_id):
    ...
    for u in updates:
        ...
        if text == "/start" or text.startswith("/start "):
            ...
            triggered = True
    return triggered, start_requested, new_max_id
...
def main():
    ...
    user_asked, start_requested, new_update_id = get_new_updates(last_update_id)
    state["last_update_id"] = new_update_id

    # Confirm the Telegram connection immediately when /start is received.
    if start_requested:
        send_welcome_message()
        print("[SENT] Telegram welcome message.")
    ...
    # 2. On-demand status reply if the user messaged the bot since the last run
    if user_asked:
